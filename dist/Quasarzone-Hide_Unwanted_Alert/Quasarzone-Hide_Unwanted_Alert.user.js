...
// ==/UserScript==
void (function (W, D) {
    const alertBox = D.querySelector('div.alert_box_wrap, div.mb_alert_box_wrap');
    if (!alertBox)
        return;
        ...
        for (const mut of muts) {
            for (const node of mut.addedNodes) {
                if (node instanceof HTMLDivElement && /(?:mb_)?alert_box/.test(node.className)) {
                    ...
                    const alertTitle = node.querySelector('.alert_title_box > p, .mb_alert_content > p');
                    if (alertTitle &&
...
//# sourceMappingURL=Quasarzone-Hide_Unwanted_Alert.user.js.map