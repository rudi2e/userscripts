...
void (function (W: Window, D: Document, L: Location): void {
  const cLog = (...text: unknown[]) => console.log('Direct Shop Link:', ...text)
  ...
  if (L.host === 'www.ppomppu.co.kr' || L.host === 'm.ppomppu.co.kr') {
    const launch = (e: HTMLElement | Document = D): void => {
      const links = e.querySelectorAll<HTMLAnchorElement>(
        'a[href^="https://s.ppomppu.co.kr"]:not(.scrap_bx_href, [data-direct-shop-link="true"])'
      )
    ...
    const commentEle = D.querySelector<HTMLDivElement>('div#quote')
